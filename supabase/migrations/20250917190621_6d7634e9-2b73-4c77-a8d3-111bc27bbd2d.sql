-- Fix ambiguous column references in Ankit functions

-- Update get_ankit_donations function to fix ambiguous column reference
...
AS $$
BEGIN
  -- Verify this is the Ankit streamer (fix ambiguous column reference)
  IF NOT EXISTS (
    SELECT 1 FROM public.streamers s
    WHERE s.id = p_streamer_id AND s.streamer_slug = 'ankit'
  ) THEN
...
-- Update get_ankit_moderation_donations function to fix ambiguous column reference
