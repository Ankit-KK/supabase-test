-- Secure donation payment information with robust access controls
...
-- Add financial data protection functions
CREATE OR REPLACE FUNCTION public.protect_financial_data()
RETURNS TRIGGER AS $$
BEGIN
  -- Prevent modification of critical financial fields after payment processing starts
  IF OLD.payment_status != 'pending' AND (
    OLD.amount != NEW.amount OR 
    OLD.order_id IS DISTINCT FROM NEW.order_id OR
    OLD.cashfree_order_id IS DISTINCT FROM NEW.cashfree_order_id
  ) THEN
    RAISE EXCEPTION 'Cannot modify financial data after payment processing has started';
  END IF;
...
-- Apply financial data protection to all donation tables
DROP TRIGGER IF EXISTS protect_ankit_financial_data ON public.ankit_donations;
CREATE TRIGGER protect_ankit_financial_data
  BEFORE UPDATE ON public.ankit_donations
  FOR EACH ROW EXECUTE FUNCTION public.protect_financial_data();

DROP TRIGGER IF EXISTS protect_chia_financial_data ON public.chia_gaming_donations;
CREATE TRIGGER protect_chia_financial_data
  BEFORE UPDATE ON public.chia_gaming_donations
  FOR EACH ROW EXECUTE FUNCTION public.protect_financial_data();

DROP TRIGGER IF EXISTS protect_newstreamer_financial_data ON public.newstreamer_donations;
CREATE TRIGGER protect_newstreamer_financial_data
