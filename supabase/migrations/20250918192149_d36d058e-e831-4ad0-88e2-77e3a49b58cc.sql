-- Step 2: Complete Database Recreation - Tables and Functions
...
DROP POLICY IF EXISTS "Anyone can view approved donations" ON public.ankit_donations;
...
DROP POLICY IF EXISTS "Anyone can view approved donations" ON public.chia_gaming_donations;
CREATE POLICY "Anyone can view approved donations" ON public.chia_gaming_donations
FOR SELECT USING (moderation_status IN ('approved', 'auto_approved') AND payment_status = 'success');

DROP POLICY IF EXISTS "Anyone can view approved donations" ON public.demostreamer_donations;
CREATE POLICY "Anyone can view approved donations" ON public.demostreamer_donations
FOR SELECT USING (moderation_status IN ('approved', 'auto_approved') AND payment_status = 'success');

-- Create essential functions with proper security
