-- Fix the get_streamer_moderation_donations function to handle authentication properly
...
BEGIN
  ...
  -- Verify access with multiple methods:
  -- 1. User owns the streamer (user_id matches)
  -- 2. User is admin email 
  -- 3. User email is allowed for this specific streamer
  -- 4. For legacy streamers (ankit, chia_gaming, etc) allow if authenticated
  IF NOT (
    ...
    -- Method 2: Admin email
    public.is_admin_email(current_user_email) OR
    -- Method 3: Email allowed for specific streamer
    (current_user_email IS NOT NULL AND public.check_streamer_email_allowed(streamer_slug_var, current_user_email)) OR
    -- Method 4: Legacy access for authenticated users on known streamers
    (auth.uid() IS NOT NULL AND current_user_email IS NOT NULL AND 
     streamer_slug_var IN ('ankit', 'chia_gaming', 'demostreamer'))
  ) THEN
