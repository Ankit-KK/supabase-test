-- Create missing RPC functions for dashboard functionality

-- Function to get streamer donations for dashboards
CREATE OR REPLACE FUNCTION get_streamer_donations(p_streamer_id uuid, p_table_name text)
RETURNS TABLE(
  id uuid,
  name text,
  amount numeric,
...
CREATE OR REPLACE FUNCTION get_streamer_moderation_donations(p_streamer_id uuid, p_table_name text)
RETURNS TABLE(
  id uuid,
  name text,
  amount numeric,
