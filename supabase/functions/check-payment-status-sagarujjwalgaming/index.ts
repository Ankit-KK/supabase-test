...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Checking SAGAR UJJWAL GAMING Razorpay payment status for order:', order_id);
    ...
    console.error('Error in check-payment-status-sagarujjwalgaming:', error);
    return new Response(
      JSON.stringify({ 
        error: error.message,
