...
serve(async (req) => {
  ...
  try {
    ...
    if (!streamer_slug || streamer_slug !== 'codelive') {
      throw new Error('Invalid streamer slug for this endpoint');
    }

    console.log(`Creating payment order for CodeLive: ${name} - ₹${amount}`);
    ...
    const cashfreeOrder = {
      ...
      customer_details: {
        ...
        customer_name: name,
        customer_email: 'donor@codelive.com',
        customer_phone: '9999999999'
      },
      order_meta: {
        return_url: `${Deno.env.get('SUPABASE_URL')?.replace('supabase.co', 'supabase.co')}/functions/v1/process-codelive-payment`,
        notify_url: `${Deno.env.get('SUPABASE_URL')?.replace('supabase.co', 'supabase.co')}/functions/v1/codelive-payment-webhook`
      }
    };
    ...
    console.error('Error in create-payment-order-codelive:', error);
