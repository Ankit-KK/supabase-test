...
serve(async (req) => {
  ...
  try {
    ...
    const orderId = `valorantpro_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    ...
    const cashfreePayload = {
      ...
      order_meta: {
        return_url: `${req.headers.get('origin')}/valorantpro?order_id=${orderId}&status={order_status}`,
