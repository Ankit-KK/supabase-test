...
serve(async (req) => {
  ...
  try {
    ...
    const orderId = `streamer39_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    ...
    const cashfreeResponse = await fetch('https://api.cashfree.com/pg/orders', {
      ...
      body: JSON.stringify({
        ...
        order_meta: {
          return_url: `${req.headers.get('origin')}/streamer39`,
        },
      }),
    });
    ...
    await supabase.from('streamer39_donations').insert({
