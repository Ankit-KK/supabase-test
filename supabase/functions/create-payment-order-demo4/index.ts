...
serve(async (req) => {
  ...
  try {
    console.log('Demo4 payment order request received');
    ...
    const orderId = `demo4_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    ...
    console.log('Demo4 donation created successfully:', orderId);
    ...
    console.error('Error in create-payment-order-demo4:', error);
    return new Response(
      JSON.stringify({ 
        success: false, 
        error: error.message 
