...
class PusherClient {
  ...
  async trigger(channel: string, event: string, data: any): Promise<any> {
    ...
    if (!response.ok) {
      ...
      console.error('[Ankit] Pusher error:', response.status, errorText);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('[Ankit] Checking payment status for:', order_id);
    ...
    if (dbError || !donation) {
      console.error('[Ankit] Donation not found:', dbError);
      throw new Error('Donation not found');
    }

    // If already successful, return immediately
    if (donation.payment_status === 'success') {
      console.log('[Ankit] Payment already successful');
      return new Response(
        JSON.stringify({
          order_id: order_id,
    ...
    if (!razorpayKeyId || !razorpayKeySecret || !donation.razorpay_order_id) {
      console.error('[Ankit] Missing Razorpay credentials or order ID');
    ...
    if (!orderResponse.ok) {
      ...
      console.error('[Ankit] Razorpay API error:', {
    ...
    console.log('[Ankit] Razorpay order status:', razorpayOrder.status);
    ...
    if (finalStatus === 'success' && donation.payment_status !== 'success') {
      console.log('[Ankit] Processing successful payment via fallback...');
      ...
      console.log(`[Ankit] Moderation: mode=${moderationMode}, hasMedia=${hasMedia}, mediaModEnabled=${streamer?.media_moderation_enabled}, shouldAutoApprove=${shouldAutoApprove}`);
      ...
      if (shouldGenerateTextTTS || shouldGenerateMediaTTS) {
        console.log(`[Ankit] Generating ${shouldGenerateMediaTTS ? 'media announcement' : 'text'} TTS via shared function...`);
        ...
        try {
          ...
          if (ttsResponse.ok) {
            ...
            if (ttsData.audioUrl) {
              ...
              console.log('[Ankit] TTS generated:', ttsAudioUrl);
            }
          } else {
            const ttsError = await ttsResponse.text();
            console.error('[Ankit] TTS generation failed:', ttsError);
          }
        } catch (ttsError) {
          console.error('[Ankit] TTS generation error:', ttsError);
        }
        ...
        console.log('[Ankit] Using silent audio for donation under threshold');
      }
      ...
      if (updateError) {
        console.error('[Ankit] Failed to update donation:', updateError);
      } else {
        console.log('[Ankit] Donation updated successfully');
      }
      ...
      console.log('[Ankit] Using Pusher group:', pusherGroup);
      ...
      if (pusherCreds.appId && pusherCreds.key && pusherCreds.secret) {
        ...
        try {
          ...
          await pusher.trigger('ankit-dashboard', 'new-donation', dashboardPayload);
          console.log('[Ankit] Dashboard notification sent');
          ...
          if (newModerationStatus === 'auto_approved') {
            ...
            await pusher.trigger('ankit-audio', 'new-audio-message', audioPayload);
            console.log('[Ankit] Audio queue notification sent');
          }

        } catch (pusherError) {
          console.error('[Ankit] Pusher error:', pusherError);
        }
        ...
        if (streamer?.goal_is_active && streamer?.goal_target_amount) {
          try {
            ...
            await pusher.trigger('ankit-goal', 'goal-progress', {
              currentAmount: totalRaised,
              targetAmount: streamer.goal_target_amount,
            });
            console.log('[Ankit] Goal progress notification sent:', totalRaised);
          } catch (goalError) {
            console.error('[Ankit] Goal progress error:', goalError);
          }
        }
      } else {
        console.warn('[Ankit] Pusher credentials not configured for group:', pusherGroup);
      }
      ...
      if (streamer?.telegram_moderation_enabled && newModerationStatus === 'pending') {
        console.log('[Ankit] Sending Telegram notification...');
        ...
        try {
          ...
          if (moderators && moderators.length > 0) {
            ...
            if (telegramBotToken) {
              const donationType = hasVoiceMessage ? '🎤 Voice' : hasHypersound ? '🔊 HyperSound' : hasMedia ? '📷 Media' : '💬 Text';
              
              for (const mod of moderators) {
                // Create callback mappings for buttons
                const approveCallback = await createCallbackMapping(supabase, donation.id, 'ankit_donations', 'approve');
                const rejectCallback = await createCallbackMapping(supabase, donation.id, 'ankit_donations', 'reject');
                const hideCallback = await createCallbackMapping(supabase, donation.id, 'ankit_donations', 'hide');
                const banCallback = await createCallbackMapping(supabase, donation.id, 'ankit_donations', 'ban');
                
                const messageText = `🎁 *New Donation for Ankit*
...
⏳ *Status:* Awaiting Moderation`;
              ...
              console.log('[Ankit] Telegram notifications sent to', moderators.length, 'moderators');
            }
          }
        } catch (telegramError) {
          console.error('[Ankit] Telegram notification error:', telegramError);
        }
      }
    } else if (finalStatus === 'failure' && donation.payment_status !== 'failure') {
      console.log('[Ankit] Updating payment status to failure');
      await supabase
        .from('ankit_donations')
    ...
    console.error('[Ankit] Error:', error);
    return new Response(
