...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Checking Jimmy Gaming Razorpay payment status for order:', order_id);
    ...
    if (orderData.status === 'paid' || payments.some((p: any) => p.status === 'captured')) {
      finalStatus = 'success';
      paymentStatus = 'success';
    } else if (orderData.status === 'attempted' && payments.some((p: any) => p.status === 'failed')) {
      finalStatus = 'failure';
    ...
    return new Response(
      JSON.stringify({
        ...
        order_amount: orderData.amount / 100,
    ...
    console.error('Error in check-payment-status-jimmygaming:', error);
    return new Response(
      JSON.stringify({ 
