...
serve(async (req) => {
  ...
  try {
    console.log('Demo3 voice upload request received');
    ...
    const filename = `demo3-voice-messages/${donation.id}_${Date.now()}.webm`;
    ...
    console.log('Demo3 voice message uploaded successfully:', voiceMessageUrl);
    ...
    console.error('Error in upload-voice-message-demo3:', error);
    return new Response(
      JSON.stringify({ 
        success: false, 
        error: error.message 
