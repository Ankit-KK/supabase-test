...
serve(async (req) => {
  ...
  try {
    console.log('Demo2 voice upload request received');
    ...
    const filename = `demo2-voice-messages/${donation.id}_${Date.now()}.webm`;
    ...
    console.log('Demo2 voice message uploaded successfully:', voiceMessageUrl);
    ...
    console.error('Error in upload-voice-message-demo2:', error);
    return new Response(
      JSON.stringify({ 
        success: false, 
        error: error.message 
