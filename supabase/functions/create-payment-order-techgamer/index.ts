...
serve(async (req) => {
  ...
  try {
    ...
    if (!streamer_slug || streamer_slug !== 'techgamer') {
      throw new Error('Invalid streamer slug for this endpoint');
    }

    console.log(`Creating payment order for TechGamer: ${name} - ₹${amount}`);
    ...
    const orderId = `techgamer_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    ...
    const cashfreeOrder = {
      ...
      customer_details: {
        ...
        customer_name: name,
        customer_email: 'donor@techgamer.com',
        customer_phone: '9999999999'
      },
      order_meta: {
        return_url: `${Deno.env.get('SUPABASE_URL')?.replace('supabase.co', 'supabase.co')}/functions/v1/process-techgamer-payment`,
        notify_url: `${Deno.env.get('SUPABASE_URL')?.replace('supabase.co', 'supabase.co')}/functions/v1/techgamer-payment-webhook`
      }
    };
    ...
    console.error('Error in create-payment-order-techgamer:', error);
