...
serve(async (req) => {
  ...
  try {
    ...
    const orderId = `craftmaster_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    ...
    const cashfreePayload = {
      ...
      order_meta: {
        return_url: `${req.headers.get('origin')}/craftmaster?order_id=${orderId}&status={order_status}`,
