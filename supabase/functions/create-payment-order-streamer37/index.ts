...
serve(async (req) => {
  ...
  try {
    ...
    const orderId = `streamer37_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    ...
    const cashfreeResponse = await fetch(`${apiUrl}/orders`, {
      ...
      body: JSON.stringify({
        ...
        order_meta: {
          return_url: `${req.headers.get('origin')}/streamer37`,
        },
      }),
    });
    ...
    await supabase.from('streamer37_donations').insert({
