...
serve(async (req) => {
  ...
  try {
    ...
    const orderId = `lofibeats_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    ...
    const cashfreePayload = {
      ...
      order_meta: {
        return_url: `${req.headers.get('origin')}/lofibeats?order_id=${orderId}&status={order_status}`,
