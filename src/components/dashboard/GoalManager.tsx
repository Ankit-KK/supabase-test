...
export const GoalManager = ({ streamerId, streamerSlug, tableName }: GoalManagerProps) => {
  ...
  const goalOverlayUrl = `${window.location.origin}/${streamerSlug}/goal-overlay`;

  const copyToClipboard = async () => {
    try {
      await navigator.clipboard.writeText(goalOverlayUrl);
      setCopySuccess(true);
      toast({
        title: 'Copied!',
        ...
        <div className="flex-1 space-y-4">
          ...
          <div className="border-t border-purple-200 dark:border-purple-700 pt-4 mt-4">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => window.open(goalOverlayUrl, '_blank')}
              className="border-purple-300 text-purple-700 hover:bg-purple-50 dark:border-purple-600 dark:text-purple-300"
            >
