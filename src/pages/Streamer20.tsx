...
const Streamer20 = () => {
  ...
  useEffect(() => {
    ...
    const fetchStreamerSettings = async () => {
      const { data } = await supabase.rpc('get_streamer_public_settings', { slug: 'streamer20' });
      if (data?.[0]) {
  ...
  const handlePaymentAfterPhone = async () => {
    ...
    try {
      ...
      const response = await supabase.functions.invoke('create-payment-order-streamer20', {
        body: {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className="focus:ring-opacity-20"
                style={{ borderColor: `${brandColor}50` }}
                disabled={isAmountLocked || donationType === 'hyperemote'}
                required
              />
              ...
              {isHyperemote && (
                <div className="text-sm p-3 rounded-lg animate-pulse" style={{ backgroundColor: `${brandColor}20`, color: brandColor }}>
                  🎉 Hyperemote! Auto-approved celebration message!
                </div>
              )}
            </div>
