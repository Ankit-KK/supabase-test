...
const DorpPlays = () => {
  ...
  const { pricing } = useStreamerPricing("dorp_plays", selectedCurrency);
    ...
    try {
      ...
      if (donationType === "voice" && voiceRecorder.audioBlob) {
        ...
        const { data, error } = await supabase.functions.invoke("upload-voice-message-direct", {
          body: { voiceData: base64, streamerSlug: "dorp_plays" },
        });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-indigo-950 via-violet-900 to-purple-900">
      <div className="absolute inset-0 bg-black/40" />

      <Card className="w-full max-w-sm bg-card/90 backdrop-blur-sm border-indigo-500/20 shadow-2xl relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/20 via-violet-500/20 to-purple-500/20 blur-xl opacity-50" />
        ...
        <CardHeader className="text-center relative z-10 pb-2">
          ...
          <p className="text-xs text-muted-foreground">Support DorpPlays with your donation</p>
        </CardHeader>

        <CardContent className="space-y-4 relative z-10">
          {/* NAME */}
          <div className="space-y-2">
            <Label className="text-indigo-300">Your Name *</Label>
            <Input
              name="name"
              value={formData.name}
          ...
          {/* AMOUNT */}
          <div className="space-y-2">
            <Label className="text-indigo-300">Amount *</Label>
