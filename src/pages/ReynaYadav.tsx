...
const ReynaYadav = () => {
  ...
  const { pricing } = useStreamerPricing('reyna_yadav', selectedCurrency);
    ...
    try {
      ...
      if (donationType === "voice" && voiceRecorder.audioBlob) {
        ...
        const { data: uploadResult, error: uploadError } = await supabase.functions.invoke(
          ...
          { body: { voiceData: voiceDataBase64, streamerSlug: "reyna_yadav" } }
        );
        if (uploadError) { console.error("Voice upload error:", uploadError); throw new Error("Failed to upload voice message"); }
      ...
      const options = {
        ...
        name: "Reyna Yadav",
        description: "Support Reyna Yadav",
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative bg-gradient-to-br from-rose-950 via-slate-900 to-rose-900">
      <div className="absolute inset-0 bg-black/20"></div>
      <Card className="w-full max-w-sm mx-auto bg-card/95 backdrop-blur-sm border-rose-500/20 shadow-2xl relative overflow-hidden">
        ...
        <CardHeader className="text-center relative z-10 pb-2">
          <CardTitle className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-rose-400 to-rose-600 bg-clip-text text-transparent">
            Reyna Yadav
          </CardTitle>
          <p className="text-muted-foreground text-xs">Support Reyna Yadav with your donation</p>
        </CardHeader>

        <CardContent className="space-y-4 relative z-10">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <label htmlFor="name" className="text-sm font-medium text-rose-400">Your Name *</label>
              <Input id="name" name="name" placeholder="Enter your name" value={formData.name}
                onChange={handleInputChange}
                className="border-rose-500/30 focus:border-rose-500 focus:ring-rose-500/20" required />
            </div>

            <div className="space-y-2">
              <label className="text-xs font-medium text-rose-400">Choose your donation type</label>
              <div className="grid grid-cols-4 gap-1.5">
                {(["text", "voice", "hypersound", "media"] as const).map(type => (
                  <button key={type} type="button" onClick={() => handleDonationTypeChange(type)}
                    className={`p-2 rounded-lg border-2 transition-all ${donationType === type
            ...
            <div className="space-y-2">
              <label htmlFor="amount" className="text-sm font-medium text-rose-400">Amount *</label>
              <div className="flex gap-2">
                <Popover open={currencyOpen} onOpenChange={setCurrencyOpen}>
                  <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" aria-expanded={currencyOpen}
                      className="w-[100px] justify-between border-rose-500/30 hover:bg-rose-500/10">
                      ...
                      <CommandList>
                        ...
                        <CommandGroup>
                          {SUPPORTED_CURRENCIES.map(currency => (
                            <CommandItem key={currency.code} value={currency.code}
                              onSelect={value => { setSelectedCurrency(value.toUpperCase()); setCurrencyOpen(false); }}>
                ...
                <Input id="amount" name="amount" type="number" placeholder="Enter amount"
                  ...
                  className={`flex-1 border-rose-500/30 focus:border-rose-500 focus:ring-rose-500/20 ${donationType === "hypersound" ? "opacity-50 cursor-not-allowed" : ""}`}
                  required min="1" />
              </div>
              {pricing.ttsEnabled && (
                <p className="text-xs text-muted-foreground">TTS above {currencySymbol}{pricing.minTts}</p>
              )}
            </div>

            {donationType === "text" && (
              <div className="space-y-2">
                <label htmlFor="message" className="text-sm font-medium text-rose-400">Your Message (Optional)</label>
                <textarea id="message" name="message" placeholder="Enter your message"
                  value={formData.message} onChange={handleInputChange}
                  maxLength={maxMessageLength} rows={3}
                  className="w-full min-h-[100px] px-3 py-2 rounded-md border border-rose-500/30 focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 bg-background text-foreground placeholder:text-muted-foreground" />
            ...
            {donationType === "voice" && (
              <div className="space-y-2">
                <label className="text-sm font-medium text-rose-400">Voice Message *</label>
                <EnhancedVoiceRecorder controller={voiceRecorder}
            ...
            {donationType === "hypersound" && (
              <div className="space-y-2">
                <Label className="text-sm font-medium text-rose-400">Select a Sound</Label>
            ...
            {donationType === "media" && (
              <div className="space-y-2">
                <Label className="text-sm font-medium text-rose-400">Upload Media</Label>
                <MediaUploader streamerSlug="reyna_yadav"
            ...
            <Button type="submit" className="w-full font-semibold py-6 bg-rose-600 hover:bg-rose-700"
              disabled={isProcessingPayment}>
              {isProcessingPayment ? "Processing..." : `Support with ${currencySymbol}${formData.amount || "0"}`}
            </Button>
          </form>
          <DonationPageFooter brandColor="#f43f5e" />
        </CardContent>
      </Card>
    </div>
  );
