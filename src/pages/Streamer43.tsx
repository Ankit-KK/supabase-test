...
const Streamer43 = () => {
  ...
  useEffect(() => {
    ...
    const fetchStreamerSettings = async () => {
      const { data } = await supabase.rpc('get_streamer_public_settings', { slug: 'streamer43' });
      if (data?.[0]) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('create-payment-order-streamer43', {
        body: {
          name: formData.name,
      ...
      const checkoutOptions = {
        ...
        returnUrl: `${window.location.origin}/streamer43`,
      };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 to-sky-100 dark:from-gray-900 dark:to-gray-800 p-4">
