...
const Wolfy = () => {
  ...
  const { pricing } = useStreamerPricing("wolfy", selectedCurrency);
    ...
    try {
      ...
      if (donationType === "voice" && voiceRecorder.audioBlob) {
        ...
        const { data, error } = await supabase.functions.invoke("upload-voice-message-direct", {
          body: { voiceData: base64, streamerSlug: "wolfy" },
        });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-amber-950 via-orange-900 to-yellow-900">
      <div className="absolute inset-0 bg-black/40" />

      <Card className="w-full max-w-sm bg-card/90 backdrop-blur-sm border-amber-500/20 shadow-2xl relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-amber-500/20 via-orange-500/20 to-yellow-500/20 blur-xl opacity-50" />
        ...
        <CardHeader className="text-center relative z-10 pb-2">
          ...
          <p className="text-xs text-muted-foreground">Support Wolfy with your donation</p>
        </CardHeader>

        <CardContent className="space-y-4 relative z-10">
          {/* NAME */}
          <div className="space-y-2">
            <Label className="text-amber-300">Your Name *</Label>
            <Input
              name="name"
              value={formData.name}
          ...
          {/* AMOUNT */}
          <div className="space-y-2">
            <Label className="text-amber-300">Amount *</Label>
