...
const STYLES = `
  ...
  /* Halftone dots on page — neon-tinted */
  .lf-page::before {
    content: ''; position: fixed; inset: 0; pointer-events: none; z-index: 0;
    background-image: radial-gradient(rgba(192,38,211,0.1) 1px, transparent 1px);
    background-size: 10px 10px;
  }

  /* Neon atmosphere blobs */
  .lf-atm {
    position: fixed; inset: 0; pointer-events: none; z-index: 1;
    background:
      radial-gradient(ellipse 70% 55% at 10% 10%, rgba(124,58,237,0.25) 0%, transparent 55%),
      radial-gradient(ellipse 60% 50% at 88% 85%, rgba(236,72,153,0.16) 0%, transparent 55%),
      radial-gradient(ellipse 45% 35% at 55% 50%, rgba(192,38,211,0.08) 0%, transparent 60%);
  }

  .lf-scale-wrap { width: 420px; transform-origin: top center; position: relative; z-index: 10; }

  /* ── Card: ink border + hard offset shadow + neon glow underneath ── */
  .lf-card {
    width: 420px; background: var(--lf-card); border-radius: 16px;
    /* Ink outline sits on top, neon glow bleeds from behind */
    border: 2.5px solid rgba(255,255,255,0.18);
    box-shadow:
      5px 5px 0px var(--lf-ink),                         /* hard ink offset */
      0 0 0 1px rgba(192,38,211,0.25),                    /* neon purple ring */
      0 0 28px rgba(124,58,237,0.3),                      /* violet bloom */
      0 0 55px rgba(192,38,211,0.15),                     /* wide purple haze */
      0 0 90px rgba(236,72,153,0.1),                      /* outer pink corona */
      0 30px 80px rgba(0,0,0,0.75);
    overflow: hidden;
    position: relative;
  }

  /* Halftone dots on card — neon purple dots */
  .lf-card::before {
  ...
  .lf-hero {
    ...
    overflow: visible;
    background: linear-gradient(135deg, rgba(124,58,237,0.22) 0%, rgba(192,38,211,0.12) 60%, transparent 100%);
    /* Ink border bottom with neon glow on it */
    border-bottom: 2.5px solid var(--lf-ink);
    box-shadow: 0 2.5px 0 0 var(--lf-ink), 0 4px 16px rgba(192,38,211,0.2);
    z-index: 1;
  }
  /* Neon top line */
  .lf-hero::before {
    ...
    box-shadow: 0 0 8px var(--lf-purple), 0 0 20px rgba(192,38,211,0.6);
  }
  ...
  /* ── Avatar: starburst with neon filter glow ── */
  .lf-avatar-wrap {
    position: relative; flex-shrink: 0; z-index: 1; width: 60px; height: 60px;
  }
  /* Gold starburst — neon drop-shadow halo */
  .lf-avatar-wrap::before {
    ...
    /* neon gold glow radiating outward */
    filter: drop-shadow(0 0 6px rgba(251,191,36,0.9)) drop-shadow(0 0 14px rgba(251,191,36,0.5));
  }
  /* Pink starburst — neon pink glow */
  .lf-avatar-wrap::after {
    ...
    opacity: 0.7;
    filter: drop-shadow(0 0 5px rgba(236,72,153,0.8)) drop-shadow(0 0 12px rgba(236,72,153,0.4));
  }
  @keyframes lf-burst {
    from { transform: translate(-50%, -50%) rotate(0deg); }
    to   { transform: translate(-50%, -50%) rotate(360deg); }
  }
  /* Avatar — ink outline + neon purple ring glow */
  .lf-avatar {
  ...
  /* ── Name: Bangers + ink outline + neon glow erupting through ── */
  ...
  .lf-name {
    ...
    text-shadow:
     ...
     -1px  1px 0 var(--lf-ink),
      2px  0   0 var(--lf-ink),
      0    2px 0 var(--lf-ink),
      /* neon glow bleeding through ink */
      0 0 12px rgba(236,72,153,0.9),
      0 0 28px rgba(192,38,211,0.7),
      0 0 50px rgba(124,58,237,0.4);
  }

  /* ── Speech bubble: white box + ink border + neon outer glow ── */
  .lf-speech {
    ...
    border-radius: 6px;
    /* Ink hard shadow + neon pink glow on the bubble */
    box-shadow:
      2px 2px 0 var(--lf-ink),
      0 0 10px rgba(236,72,153,0.5),
      0 0 22px rgba(192,38,211,0.25);
    white-space: nowrap;
  }
  /* Ink tail */
  .lf-speech::before {
  ...
  /* ── Live badge: ink pill + neon green bleeding out ── */
  ...
  .lf-live {
    ...
    margin-left: auto;
    /* neon green glow bleeding through ink border */
    box-shadow:
      2px 2px 0 var(--lf-ink),
  ...
  .lf-live-dot {
    ...
    box-shadow: 0 0 6px var(--lf-green), 0 0 12px rgba(52,211,153,0.6);
  }
  .lf-live-text {
    font-family: 'Bangers', cursive; font-size: 12px; letter-spacing: 0.1em; color: var(--lf-green);
    text-shadow: 0 0 8px var(--lf-green), 1px 1px 0 rgba(0,0,0,0.8);
  }

  /* ── Body ── */
  .lf-body { padding: 14px 18px 16px; display: flex; flex-direction: column; gap: 11px; position: relative; z-index: 1; }

  /* Labels: Bangers + ink outline + neon color */
  .lf-lbl {
    font-family: 'Bangers', cursive;
    ...
    text-shadow:
     ...
     -1px -1px 0 var(--lf-ink),
      1px -1px 0 var(--lf-ink),
     -1px  1px 0 var(--lf-ink),
      0 0 10px rgba(192,38,211,0.6);
  }

  /* ── Inputs: ink offset at rest, ink + neon ring on focus ── */
  .lf-iw input {
    width:100% !important; background:rgba(255,255,255,0.04) !important;
    border: 2px solid rgba(124,58,237,0.4) !important; border-radius:8px !important;
  ...
  .lf-iw input:focus {
    ...
    box-shadow:
      ...
      0 0 16px rgba(192,38,211,0.35) !important;
  }
  ...
  /* ══════════════════════
     3D TYPE BUTTONS
     — ink offset when idle
     — ink collapses + neon erupts when active
  ══════════════════════ */
  ...
  .lf-tb-pu .lf-tb-face {
    ...
    box-shadow: 2px 2px 0 var(--lf-ink), 0 0 8px rgba(124,58,237,0.3);
  }
  .lf-tb-pu::after { background:#2d006b; border:2px solid rgba(124,58,237,0.35); }
  .lf-tb-pu.lf-on .lf-tb-face {
    transform:translateY(0);
    background:linear-gradient(160deg,rgba(124,58,237,0.4),rgba(100,0,180,0.65));
    border-color:var(--lf-violet);
    /* ink collapses to 1px offset, neon erupts */
    box-shadow:
      1px 1px 0 var(--lf-ink),
  ...
  .lf-tb-pk .lf-tb-face {
    ...
    box-shadow: 2px 2px 0 var(--lf-ink), 0 0 8px rgba(236,72,153,0.3);
  }
  .lf-tb-pk::after { background:#6b0040; border:2px solid rgba(236,72,153,0.35); }
  .lf-tb-pk.lf-on .lf-tb-face {
    transform:translateY(0);
    background:linear-gradient(160deg,rgba(236,72,153,0.4),rgba(180,0,100,0.6));
    border-color:var(--lf-pink);
    box-shadow:
      1px 1px 0 var(--lf-ink),
  ...
  .lf-tb-mg .lf-tb-face {
    ...
    box-shadow: 2px 2px 0 var(--lf-ink), 0 0 8px rgba(232,121,249,0.3);
  }
  .lf-tb-mg::after { background:#5a0075; border:2px solid rgba(232,121,249,0.35); }
  .lf-tb-mg.lf-on .lf-tb-face {
    transform:translateY(0);
    background:linear-gradient(160deg,rgba(232,121,249,0.4),rgba(160,0,190,0.6));
    border-color:var(--lf-magenta);
    box-shadow:
      1px 1px 0 var(--lf-ink),
      inset 0 2px 5px rgba(0,0,0,0.3),
      0 0 16px rgba(232,121,249,0.9),
      0 0 32px rgba(232,121,249,0.5),
      0 0 55px rgba(232,121,249,0.25);
  }
  ...
  .lf-tb-rs .lf-tb-face {
    ...
    box-shadow: 2px 2px 0 var(--lf-ink), 0 0 8px rgba(251,113,133,0.3);
  }
  .lf-tb-rs::after { background:#6b0028; border:2px solid rgba(251,113,133,0.35); }
  .lf-tb-rs.lf-on .lf-tb-face {
    transform:translateY(0);
    background:linear-gradient(160deg,rgba(251,113,133,0.4),rgba(180,0,60,0.6));
    border-color:var(--lf-rose);
    box-shadow:
      1px 1px 0 var(--lf-ink),
      inset 0 2px 5px rgba(0,0,0,0.3),
      0 0 16px rgba(251,113,133,0.9),
      0 0 32px rgba(251,113,133,0.5),
      0 0 55px rgba(251,113,133,0.25);
  }

  /* Button label: Bangers + ink outline + neon on active */
  ...
  .lf-cur:hover {
    ...
    box-shadow: 2px 2px 0 var(--lf-ink), 0 0 10px rgba(192,38,211,0.35) !important;
  }

  .lf-div {
    height:2px;
    background:linear-gradient(90deg,transparent,rgba(124,58,237,0.6),rgba(236,72,153,0.4),transparent);
    box-shadow: 0 0 8px rgba(192,38,211,0.4);
  }

  /* Sub panels — ink border + neon color-matched glow */
  .lf-sp { border-radius:10px; padding:10px 12px; }
  .lf-sp-pk {
    background:rgba(236,72,153,0.06); border:2px solid rgba(236,72,153,0.45);
    box-shadow: 2px 2px 0 rgba(0,0,0,0.5), 0 0 14px rgba(236,72,153,0.2);
  }
  .lf-sp-mg {
    background:rgba(232,121,249,0.06); border:2px solid rgba(232,121,249,0.45);
    box-shadow: 2px 2px 0 rgba(0,0,0,0.5), 0 0 14px rgba(232,121,249,0.2);
  }
  .lf-sp-rs {
    background:rgba(251,113,133,0.06); border:2px solid rgba(251,113,133,0.4);
    box-shadow: 2px 2px 0 rgba(0,0,0,0.5), 0 0 14px rgba(251,113,133,0.2);
  }

  /* ══════════════════════
     POW SUBMIT BUTTON
     — jagged clip + ink shadow block + neon gradient glow
  ══════════════════════ */
  ...
  .lf-btn {
    ...
    /* POW jagged polygon */
    clip-path: polygon(0% 8%, 3% 0%, 97% 0%, 100% 8%, 100% 92%, 97% 100%, 3% 100%, 0% 92%);
    /* ink text shadow + neon fill glow */
    text-shadow:
      2px 2px 0 var(--lf-ink),
  ...
  .lf-btn:hover:not(:disabled) {
    box-shadow:
      inset 0 1px 0 rgba(255,200,240,0.2),
      0 0 30px rgba(192,38,211,0.9),
      0 0 60px rgba(236,72,153,0.5),
      0 0 90px rgba(124,58,237,0.3);
  }
  .lf-btn:active:not(:disabled) { transform:translateY(0px) !important; box-shadow: inset 0 2px 8px rgba(0,0,0,0.5), 0 0 15px rgba(192,38,211,0.5) !important; }
  ...
  .lf-hint {
    ...
    text-shadow: 1px 1px 0 var(--lf-ink), 0 0 8px rgba(232,121,249,0.6);
  }
  ...
  /* Rewards — jagged comic badge + neon gold glow */
  .lf-rewards-btn {
    ...
    box-shadow: 2px 2px 0 rgba(0,0,0,0.6), 0 0 12px rgba(251,191,36,0.2);
    text-shadow: 1px 1px 0 rgba(0,0,0,0.7), 0 0 8px rgba(251,191,36,0.5);
    clip-path: polygon(0% 8%, 3% 0%, 97% 0%, 100% 8%, 100% 92%, 97% 100%, 3% 100%, 0% 92%);
  }
  .lf-rewards-btn:hover {
    background:rgba(251,191,36,0.15); border-color:var(--lf-gold);
    box-shadow: 3px 3px 0 var(--lf-ink), 0 0 22px rgba(251,191,36,0.45), 0 0 40px rgba(251,191,36,0.2);
    text-shadow: 1px 1px 0 var(--lf-ink), 0 0 14px rgba(251,191,36,0.8);
  }
`;
                        ...
                        <div className="lf-tb-face">
                          ...
                          <span className="lf-tb-name" style={{
                            ...
                            textShadow: donationType === t.key
                              ? `1px 1px 0 #000, 0 0 10px ${t.nc}, 0 0 20px ${t.nc}`
                ...
                {donationType === "text" && (
                  <div className="lf-fu">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                      ...
                      <span style={{ fontFamily: "'Bangers',cursive", fontSize: 11, letterSpacing: '0.08em', color: msgClr, textShadow: `1px 1px 0 #000, 0 0 8px ${msgClr}` }}>{formData.message.length}/{maxMessageLength}</span>
                    </div>
                ...
                {donationType === "hypersound" && (
                  <div className="lf-fu lf-sp lf-sp-mg">
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      ...
                      <span style={{ fontFamily: "'Bangers',cursive", fontSize: 14, letterSpacing: '0.08em', color: 'var(--lf-magenta)', textShadow: '1px 1px 0 #000, 0 0 10px rgba(232,121,249,0.7)' }}>HyperSounds</span>
                    </div>
                ...
                {donationType === "media" && (
                  <div className="lf-fu lf-sp lf-sp-rs">
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      ...
                      <span style={{ fontFamily: "'Bangers',cursive", fontSize: 14, letterSpacing: '0.08em', color: 'var(--lf-rose)', textShadow: '1px 1px 0 #000, 0 0 10px rgba(251,113,133,0.7)' }}>Media Upload</span>
                    </div>
