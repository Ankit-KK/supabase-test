...
const TechGamer = () => {
    ...
    const fetchStreamerSettings = async () => {
      try {
        const { data, error } = await supabase
          .rpc('get_streamer_public_settings', { slug: 'techgamer' });
  ...
  const handlePaymentAfterPhone = async () => {
    ...
    try {
      ...
      const response = await supabase.functions.invoke('create-payment-order-techgamer', {
        body: {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background to-muted flex items-center justify-center p-4">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-20 right-20 w-40 h-40 bg-green-600/10 rounded-full blur-3xl animate-pulse"></div>
        <div className="absolute top-1/2 left-1/4 w-24 h-24 bg-green-400/10 rounded-full blur-2xl animate-pulse"></div>
      </div>

      <Card className="w-full max-w-md mx-auto bg-card/95 backdrop-blur-sm border-green-500/20 shadow-2xl relative overflow-hidden">
        {/* Card glow effect */}
        <div className="absolute inset-0 bg-gradient-to-r from-green-500/20 via-green-600/20 to-green-400/20 opacity-50 blur-xl"></div>
        ...
        <CardHeader className="text-center relative z-10">
          ...
          <CardTitle className="text-2xl font-bold bg-gradient-to-r from-green-500 to-green-600 bg-clip-text text-transparent">
            TechGamer
          </CardTitle>
          <p className="text-muted-foreground text-sm">
            Support TechGamer with your donation
          </p>
        </CardHeader>
        ...
        <CardContent className="space-y-6 relative z-10">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <label htmlFor="name" className="text-sm font-medium text-green-500">
                Your Name *
              </label>
              <Input
                id="name"
            ...
            {donationType === 'message' && (
              <div className="space-y-2">
                <label htmlFor="message" className="text-sm font-medium text-green-500">
                  Message *
                </label>
                <textarea
                  id="message"
                  ...
                  onChange={handleInputChange}
                  className="w-full p-3 border border-green-500/30 rounded-lg bg-background/50 backdrop-blur-sm focus:border-green-500 focus:ring-2 focus:ring-green-500/20 focus:outline-none resize-none"
                  rows={3}
                  maxLength={500}
                  required
                />
            ...
            <Button
              type="submit"
              className="w-full bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-semibold py-3 px-6 rounded-lg shadow-lg transition-all duration-200 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
