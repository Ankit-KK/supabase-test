...
const Streamer29 = () => {
  ...
  useEffect(() => {
    ...
    const fetchStreamerSettings = async () => {
      const { data } = await supabase.rpc('get_streamer_public_settings', { slug: 'streamer29' });
      if (data?.[0]) {
  ...
  const handlePaymentWithPhone = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('create-payment-order-streamer29', {
        body: {
          name: formData.name,
          amount: amount,
      ...
      const checkoutOptions = {
        ...
        returnUrl: `${window.location.origin}/streamer29`,
      };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-50 via-cyan-50 to-emerald-50 dark:from-gray-900 dark:to-gray-800 p-4">
        ...
        <Card className="border-teal-200 dark:border-teal-800 shadow-xl">
          <CardHeader className="text-center space-y-4 pb-8">
            <div className="flex justify-center">
              <div className="p-4 bg-teal-100 dark:bg-teal-900/20 rounded-full">
                <Gift className="w-12 h-12 text-teal-600 dark:text-teal-400" />
              </div>
            </div>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {donationType === 'message' && (
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    rows={4}
                    className="border-teal-200 focus:border-teal-400 focus:ring-teal-400 resize-none"
              ...
              {donationType === 'hyperemote' && (
                <div className="p-4 bg-teal-50 dark:bg-teal-900/20 border-2 border-teal-300 dark:border-teal-700 rounded-lg">
                  <div className="flex items-start gap-3">
                    <Gift className="w-6 h-6 text-teal-600 dark:text-teal-400 flex-shrink-0 mt-1" />
                    <div>
              ...
              <Button
                type="submit"
                className="w-full h-12 text-lg font-semibold bg-teal-600 hover:bg-teal-700 text-white shadow-lg hover:shadow-xl transition-all duration-200"
