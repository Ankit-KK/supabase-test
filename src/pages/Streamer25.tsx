...
const Streamer25 = () => {
  ...
  useEffect(() => {
    ...
    const fetchStreamerSettings = async () => {
      const { data } = await supabase.rpc('get_streamer_public_settings', { slug: 'streamer25' });
      if (data?.[0]) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('create-payment-order-streamer25', {
        body: {
          name: formData.name,
      ...
      const checkoutOptions = {
        ...
        returnUrl: `${window.location.origin}/streamer25`,
      };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-fuchsia-100 dark:from-gray-900 dark:to-gray-800 p-4">
      <div className="container mx-auto max-w-2xl py-8">
        <Card className="border-purple-200 dark:border-purple-800">
          <CardHeader className="text-center">
            <CardTitle className="text-3xl font-bold" style={{ color: '#a855f7' }}>
              Support Streamer 25
            </CardTitle>
