...
import { STREAMER_COLORS } from '@/utils/streamerPageTemplate';

const Streamer25 = () => {
  const navigate = useNavigate();
  const { toast } = useToast();
  const { getCharacterLimit, getVoiceDuration } = useDonationLimits();
  const brandColor = STREAMER_COLORS[25];
  ...
  const handlePhoneSubmit = async () => {
    ...
    try {
      ...
      const response = await supabase.functions.invoke('create-payment-order-streamer25', {
        body: {
          name: formData.name,
          amount,
        ...
        <CardHeader className="text-center space-y-4 p-0">
          <div className="flex justify-center">
            <div className="p-4 rounded-full shadow-lg" style={{ backgroundColor: `${brandColor}20` }}>
              <Heart className="h-12 w-12" style={{ color: brandColor }} />
            </div>
          </div>
