...
const Demo3 = () => {
  ...
  useEffect(() => {
    ...
    const fetchStreamerSettings = async () => {
      try {
        const { data, error } = await supabase.rpc('get_streamer_public_settings', { slug: 'demo3' });
  ...
  const handlePaymentAfterPhone = async () => {
    ...
    try {
      ...
      const response = await supabase.functions.invoke('create-payment-order-demo3', {
        body: {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background to-muted flex items-center justify-center p-4">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-20 right-20 w-40 h-40 bg-emerald-600/10 rounded-full blur-3xl animate-pulse"></div>
      </div>

      <Card className="w-full max-w-md mx-auto bg-card/95 backdrop-blur-sm border-emerald-500/20 shadow-2xl relative overflow-hidden">
        ...
        <CardHeader className="text-center relative z-10">
          ...
          <p className="text-muted-foreground text-sm">Support Demo Streamer 3 with your donation</p>
        </CardHeader>

        <CardContent className="space-y-6 relative z-10">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <label htmlFor="name" className="text-sm font-medium text-emerald-500">Your Name *</label>
              <Input id="name" name="name" placeholder="Enter your name" value={formData.name}
                onChange={handleInputChange} className="border-emerald-500/30 focus:border-emerald-500" required />
            </div>

            <div className="space-y-3">
              <label className="text-sm font-medium text-emerald-500">Choose your donation type</label>
              <div className="grid grid-cols-3 gap-2">
                <button type="button" onClick={() => handleDonationTypeChange('message')}
                  className={`p-3 rounded-lg border-2 transition-all ${donationType === 'message' ? 'border-emerald-500 bg-emerald-500/10' : 'border-emerald-500/30'}`}>
                ...
                <button type="button" onClick={() => handleDonationTypeChange('voice')}
                  className={`p-3 rounded-lg border-2 transition-all ${donationType === 'voice' ? 'border-emerald-500 bg-emerald-500/10' : 'border-emerald-500/30'}`}>
                ...
                <button type="button" onClick={() => handleDonationTypeChange('hyperemote')}
                  className={`p-3 rounded-lg border-2 transition-all ${donationType === 'hyperemote' ? 'border-emerald-500 bg-emerald-500/10' : 'border-emerald-500/30'}`}>
            ...
            <div className="space-y-2">
              <label htmlFor="amount" className="text-sm font-medium text-emerald-500">Amount (₹) *</label>
            ...
            {donationType === 'message' && (
              <div className="space-y-2">
                <label htmlFor="message" className="text-sm font-medium text-emerald-500">Your Message *</label>
