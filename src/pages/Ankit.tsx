    ...
    <div className="min-h-screen bg-transparent flex items-center justify-center p-4 relative">
      ...
      <Card className="w-full max-w-sm mx-auto bg-transparent border-blue-500/20 shadow-2xl relative overflow-hidden z-10">
        ...
        <CardHeader className="text-center relative z-10 pb-2">
          ...
          <CardTitle className="text-2xl sm:text-2xl font-bold bg-gradient-to-r from-blue-500 to-blue-600 bg-clip-text text-transparent">
            Ankit
          </CardTitle>
          <p className="text-blue-200/90 text-xs">Support Ankit with your donation</p>
        </CardHeader>
        ...
        <CardContent className="space-y-4 relative z-10">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-3">
              <label className="text-sm font-medium text-blue-500">Choose your donation type</label>
              <div className="grid grid-cols-4 gap-2">
                <button
                  type="button"
                  onClick={() => handleDonationTypeChange("message")}
                  className={`p-2 rounded-lg border-2 transition-all ${donationType === "message" ? "border-blue-500 bg-blue-500/10" : "border-blue-500/30 hover:border-blue-500/50"}`}
                >
                  <div className="text-center">
                    <div className="text-base mb-1">💬</div>
                    <div className="font-medium text-[10px] text-blue-100">Text</div>
                    <div className="text-[9px] text-blue-300/80">
                      Min: {getCurrencySymbol(formData.currency)}
                      {getCurrencyMinimums(formData.currency).minText}
                    </div>
                  </div>
                </button>
                <button
                  ...
                  className={`p-2 rounded-lg border-2 transition-all ${donationType === "voice" ? "border-blue-500 bg-blue-500/10" : "border-blue-500/30 hover:border-blue-500/50"}`}
                >
                  <div className="text-center">
                    <div className="text-base mb-1">🎤</div>
                    ...
                    <div className="text-[9px] text-blue-300/80">
                      Min: {getCurrencySymbol(formData.currency)}
                      {getCurrencyMinimums(formData.currency).minVoice}
                    </div>
                  </div>
                </button>
                <button
                ...
                >
                  <div className="text-center">
                    ...
                    <div className="text-[9px] text-orange-300/80">
                      Min: {getCurrencySymbol(formData.currency)}
                      {getCurrencyMinimums(formData.currency).minHypersound}
                    </div>
                  </div>
                </button>
                <button
                  type="button"
                  onClick={() => handleDonationTypeChange("image")}
                  className={`p-2 rounded-lg border-2 transition-all ${donationType === "image" ? "border-purple-500 bg-purple-500/10" : "border-purple-500/30 hover:border-purple-500/50"}`}
                >
                  <div className="text-center">
                    <div className="text-base mb-1">📷</div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <Input
                  ...
                  placeholder={
                    donationType === "message"
                      ? `Min: ${getCurrencyMinimums(formData.currency).minText}`
                      : donationType === "voice"
                        ? `Min: ${getCurrencyMinimums(formData.currency).minVoice}`
                        : `Min: ${getCurrencyMinimums(formData.currency).minHypersound}`
                  }
              ...
              {formData.currency === "INR" && donationType === "message" && (
              ...
              {donationType === "hypersound" && (
                <p className="text-xs text-muted-foreground">
                  HyperSounds start at {getCurrencySymbol(formData.currency)}
                  {getCurrencyMinimums(formData.currency).minHypersound}
                </p>
              )}
            </div>
            ...
            {donationType === "voice" && (
              <div className="space-y-3">
                <label className="text-sm font-medium text-blue-500">Record Voice Message *</label>
                <VoiceRecorder
            ...
            >
              {isProcessing ? (
                ...
                <span className="flex items-center justify-center space-x-2">
                  ...
                  <span>
                    Donate {getCurrencySymbol(formData.currency)}
                    {formData.amount || "0"}
                  </span>
                </span>
              )}
