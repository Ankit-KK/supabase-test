...
const STYLES = `
  ...
  @keyframes v-shift { 0%{background-position:0% 0%} 100%{background-position:200% 0%} }

  /* Neon blobs behind hero */
  .v-hero-blob1 {
    position: absolute; top: -40px; left: -40px;
    width: 180px; height: 180px; border-radius: 50%;
    background: radial-gradient(circle, rgba(170,0,255,0.35) 0%, transparent 65%);
    pointer-events: none;
  }
  .v-hero-blob2 {
    position: absolute; top: -20px; right: -40px;
    width: 150px; height: 150px; border-radius: 50%;
    background: radial-gradient(circle, rgba(255,0,153,0.3) 0%, transparent 65%);
    pointer-events: none;
  }
  ...
  .v-iw input:focus {
    ...
    box-shadow: 0 0 0 3px rgba(0,238,255,0.15), 0 0 20px rgba(0,238,255,0.15) !important;
  }
  ...
  .v-ta:focus {
    ...
    box-shadow: 0 0 0 3px rgba(0,238,255,0.15), 0 0 20px rgba(0,238,255,0.15);
  }
  ...
  /* ── Type buttons — BIG vivid neon ── */
  .v-types { display: grid; grid-template-columns: repeat(4,1fr); gap: 7px; }
  .v-tb {
    padding: 11px 4px 10px; text-align: center;
    background: rgba(255,255,255,0.04);
    border: 1.5px solid rgba(255,255,255,0.1);
    border-radius: 14px;
    cursor: pointer; transition: all .18s;
  }
  .v-tb:hover { background: rgba(255,255,255,0.08); transform: translateY(-2px); }

  /* Each active type bursts with its color */
  .v-tb-cy.v-on {
    border-color: var(--cyan);
    background: rgba(0,238,255,0.12);
    box-shadow: 0 0 14px rgba(0,238,255,0.55), 0 0 30px rgba(0,238,255,0.2), inset 0 0 14px rgba(0,238,255,0.08);
  }
  .v-tb-pk.v-on {
    border-color: var(--hot-pink);
    background: rgba(255,0,153,0.12);
    box-shadow: 0 0 14px rgba(255,0,153,0.55), 0 0 30px rgba(255,0,153,0.2), inset 0 0 14px rgba(255,0,153,0.08);
  }
  .v-tb-or.v-on {
    border-color: var(--orange);
    background: rgba(255,102,0,0.12);
    box-shadow: 0 0 14px rgba(255,102,0,0.55), 0 0 30px rgba(255,102,0,0.2), inset 0 0 14px rgba(255,102,0,0.08);
  }
  .v-tb-pu.v-on {
    border-color: var(--purple);
    background: rgba(170,0,255,0.12);
    box-shadow: 0 0 14px rgba(170,0,255,0.55), 0 0 30px rgba(170,0,255,0.2), inset 0 0 14px rgba(170,0,255,0.08);
  }
  ...
  .v-cur:hover {
    border-color: var(--cyan) !important;
    box-shadow: 0 0 12px rgba(0,238,255,0.2) !important;
  }

  /* Divider */
  .v-div {
    height: 1px;
    background: linear-gradient(90deg, transparent, rgba(255,0,153,0.35), rgba(0,238,255,0.3), transparent);
    box-shadow: 0 0 6px rgba(255,0,153,0.1);
  }

  /* Sub panels — vivid colored borders */
  .v-sp { border-radius: 14px; padding: 14px 16px; }
  .v-sp-or {
    background: rgba(255,102,0,0.07);
    border: 1.5px solid rgba(255,102,0,0.45);
    box-shadow: 0 0 18px rgba(255,102,0,0.12), inset 0 0 18px rgba(255,102,0,0.04);
  }
  .v-sp-pu {
    background: rgba(170,0,255,0.07);
    border: 1.5px solid rgba(170,0,255,0.45);
    box-shadow: 0 0 18px rgba(170,0,255,0.12), inset 0 0 18px rgba(170,0,255,0.04);
  }

  /* ── Submit — the brightest thing on the page ── */
  .v-btn {
    ...
    box-shadow:
      ...
      0 0 40px rgba(255,0,153,0.25),
      0 0 80px rgba(170,0,255,0.15),
      0 6px 24px rgba(0,0,0,0.5);
  }
  .v-btn:hover:not(:disabled) {
    transform: translateY(-2px) scale(1.01);
    box-shadow:
      0 0 24px rgba(255,0,153,0.8),
      0 0 60px rgba(255,0,153,0.35),
      0 0 100px rgba(170,0,255,0.2),
      0 10px 32px rgba(0,0,0,0.6);
  }
  .v-btn:active:not(:disabled) { transform: translateY(0) scale(1); }
  ...
  /* Spinner */
  @keyframes v-sp { to{transform:rotate(360deg);} }
  .v-spin { width:16px; height:16px; border:2px solid rgba(255,255,255,0.35); border-top-color:#fff; border-radius:50%; display:inline-block; animation:v-sp .65s linear infinite; }
            ...
            <div className="v-body">
                  ...
                  {TYPES.map(t=>(
                    <button key={t.key} type="button"
                      ...
                      <span className="v-tb-name" style={{
                        color: donationType===t.key ? t.nc : 'rgba(255,255,255,0.38)',
              ...
              <div>
                ...
                {donationType==='message'&&pricing.ttsEnabled&&(
                  <p className="v-hint">⚡ TTS voice above {sym}{pricing.minTts}</p>
                )}
                {donationType==='voice'&&currentAmount>=pricing.minVoice&&(
                  <p className="v-hint">⏱ {getVoiceDuration(currentAmount)}s{formData.currency==='INR'&&currentAmount<200?' · ₹200+ for 20s, ₹250+ for 30s':''}</p>
                )}
              </div>
              ...
              {donationType==='message'&&(
                <div className="v-fu">
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
                    ...
                    <span style={{fontSize:11,fontWeight:800,color:charClr,textShadow:`0 0 8px ${charClr}`}}>
                  ...
                  <textarea id="message" name="message"
                    ...
                    className="v-ta" rows={3} maxLength={charLimit} required/>
                  <div className="v-cbar">
                    <div className="v-cbar-fill" style={{width:`${charPct}%`,background:charClr,boxShadow:`0 0 8px ${charClr}`}}/>
                  </div>
                </div>
              )}
              ...
              {donationType==='hypersound'&&(
                <div className="v-fu v-sp v-sp-or">
                  <div style={{display:'flex',alignItems:'center',gap:9,marginBottom:10}}>
                    ...
                    <span style={{fontSize:14,fontWeight:900,color:'var(--orange)',textShadow:'0 0 10px var(--orange), 0 0 20px rgba(255,102,0,0.4)'}}>HyperSounds</span>
                  </div>
                  <p style={{fontSize:13,fontWeight:600,color:'rgba(255,255,255,0.5)',marginBottom:12}}>
                    Pick a sound to blast live on stream.
                  </p>
              ...
              {donationType==='image'&&(
                <div className="v-fu v-sp v-sp-pu">
                  <div style={{display:'flex',alignItems:'center',gap:9,marginBottom:10}}>
                    ...
                    <span style={{fontSize:14,fontWeight:900,color:'var(--purple)',textShadow:'0 0 10px var(--purple), 0 0 20px rgba(170,0,255,0.4)'}}>Image Upload</span>
                    <span style={{fontSize:9,fontWeight:800,color:'var(--yellow)',border:'1.5px solid rgba(255,228,0,0.3)',borderRadius:20,padding:'1px 8px',marginLeft:4,textShadow:'0 0 6px var(--yellow)'}}>DEMO</span>
                  </div>
