...
import { Copy, Check, Music, RefreshCw, AlertCircle, Info } from 'lucide-react';
...
const AnkitMediaSourcePlayer = () => {
  ...
  const { config } = usePusherConfig('ankit');
  ...
  const validateToken = async () => {
    ...
    try {
      ...
      if (!error && data?.[0]?.is_valid && data[0].streamer_slug === 'ankit') {
  ...
  const mediaSourceUrl = `${SUPABASE_PROJECT_URL}/functions/v1/get-current-audio-ankit?token=${obsToken}`;
      ...
      <div className="max-w-2xl mx-auto space-y-6">
          ...
          <CardContent className="space-y-4">
            ...
            {obsToken && (
              <Badge variant={isValidToken ? 'default' : 'destructive'}>
                {isValidToken ? '✓ Valid Token' : '✗ Invalid or Not Ankit Token'}
              </Badge>
            )}
          </CardContent>
        </Card>
        ...
        {isValidToken && (
          <Card className="border-primary">
            <CardHeader className="pb-3">
              <CardTitle className="text-lg text-primary">Media Source URL</CardTitle>
              <CardDescription>
        ...
        <Card>
          ...
          <CardContent className="space-y-4 text-sm">
            <ol className="list-decimal list-inside space-y-3 text-muted-foreground">
              ...
              <li>
                Name it something like <strong>"Ankit Audio Alerts"</strong>
              </li>
              <li>
        ...
        <Card className="border-dashed">
          <CardContent className="pt-6">
            <p className="text-sm text-muted-foreground text-center">
              ...
              <a href="/ankit/audio-player" className="text-primary underline">
