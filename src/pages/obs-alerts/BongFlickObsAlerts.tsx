...
const BongFlickObsAlerts = () => {
  ...
  const { config: pusherConfig, loading, error } = usePusherConfig("bongflick");
    ...
    <div className="min-h-screen bg-transparent">
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="fixed bottom-4 left-4 bg-black/80 text-white p-2 rounded text-xs space-y-1">
          ...
          <button onClick={triggerTestAlert} className="mt-2 px-2 py-1 bg-green-600 rounded hover:bg-green-700">
            Test Alert
          </button>
        </div>
      )}
