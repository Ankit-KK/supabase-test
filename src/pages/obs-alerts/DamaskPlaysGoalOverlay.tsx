...
const DamaskPlaysGoalOverlay = () => {
  ...
  useEffect(() => {
    const fetchPusherConfig = async () => {
      const { data, error } = await supabase.functions.invoke('get-pusher-config', {
        body: { streamer_slug: 'damask_plays' }
      });
      if (!error && data) {
  ...
  useEffect(() => {
    const fetchGoalData = async () => {
      const { data: streamerData } = await supabase
        ...
        .eq('id', '33eefe1f-1647-4901-84a0-527c3cad21bc')
        .single();
  ...
  useEffect(() => {
    ...
    const channel = pusher.subscribe('damask_plays-goal');
    ...
    return () => {
      channel.unbind_all();
      pusher.unsubscribe('damask_plays-goal');
      pusher.disconnect();
    };
  }, [pusherConfig]);
...
export default DamaskPlaysGoalOverlay;
