...
const SIZZORS_STREAMER_ID = '33a3dceb-9b24-423c-b34c-30990ea3d6a5';
...
const SizzorsGoalOverlay = () => {
  ...
  useEffect(() => {
    ...
    const goalChannel = pusher.subscribe('sizzors-goal');
    goalChannel.bind('goal-progress', (data: { currentAmount: number }) => {
      setCurrentAmount(data.currentAmount);
    });

    const settingsChannel = pusher.subscribe('sizzors-settings');
    ...
    return () => {
      if (pusherRef.current) {
        try {
          ...
          pusher.unsubscribe('sizzors-goal');
          pusher.unsubscribe('sizzors-settings');
...
export default SizzorsGoalOverlay;
