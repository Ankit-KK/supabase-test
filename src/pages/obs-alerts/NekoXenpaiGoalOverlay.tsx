...
const NEKO_XENPAI_STREAMER_ID = '65f4932a-070e-44ff-8852-b9682cbc7f1c';
...
const NekoXenpaiGoalOverlay = () => {
  ...
  useEffect(() => {
    const fetchPusherConfig = async () => {
      const { data, error } = await supabase.functions.invoke('get-pusher-config', {
        body: { streamer_slug: 'neko_xenpai' }
      });
      if (!error && data) {
  ...
  useEffect(() => {
    ...
    const goalChannel = pusher.subscribe('neko_xenpai-goal');
    goalChannel.bind('goal-progress', (data: { currentAmount: number }) => {
      setCurrentAmount(data.currentAmount);
    });

    const settingsChannel = pusher.subscribe('neko_xenpai-settings');
    ...
    return () => {
      if (pusherRef.current) {
        try {
          ...
          pusher.unsubscribe('neko_xenpai-goal');
          pusher.unsubscribe('neko_xenpai-settings');
...
export default NekoXenpaiGoalOverlay;
