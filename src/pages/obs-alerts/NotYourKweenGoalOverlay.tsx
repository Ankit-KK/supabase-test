...
const NOTYOURKWEEN_STREAMER_ID = 'c7064442-4ab9-4c00-a00c-c79121250804';
...
const NotYourKweenGoalOverlay = () => {
  ...
  useEffect(() => {
    const fetchPusherConfig = async () => {
      const { data, error } = await supabase.functions.invoke('get-pusher-config', {
        body: { streamer_slug: 'notyourkween' }
      });
      if (!error && data) {
  ...
  useEffect(() => {
    const fetchGoalData = async () => {
      const { data: streamerData } = await supabase
        ...
        .eq('id', NOTYOURKWEEN_STREAMER_ID)
        .single();
  ...
  useEffect(() => {
    ...
    const goalChannel = pusher.subscribe('notyourkween-goal');
    goalChannel.bind('goal-progress', (data: { currentAmount: number }) => {
      setCurrentAmount(data.currentAmount);
    });

    const settingsChannel = pusher.subscribe('notyourkween-settings');
    ...
    return () => {
      if (pusherRef.current) {
        try {
          ...
          pusher.unsubscribe('notyourkween-goal');
          pusher.unsubscribe('notyourkween-settings');
...
export default NotYourKweenGoalOverlay;
