...
const MRIQMASTER_STREAMER_ID = '86294d1a-dba1-4f56-b339-503a78d5543a';
...
const MrIqmasterGoalOverlay = () => {
  ...
  useEffect(() => {
    ...
    const goalChannel = pusher.subscribe('mriqmaster-goal');
    goalChannel.bind('goal-progress', (data: { currentAmount: number }) => {
      setCurrentAmount(data.currentAmount);
    });

    const settingsChannel = pusher.subscribe('mriqmaster-settings');
    ...
    return () => {
      if (pusherRef.current) {
        try {
          ...
          pusher.unsubscribe('mriqmaster-settings');
