...
const SAGARUJJWALGAMING_STREAMER_ID = '4836e53e-0677-4432-ab79-46ad98203625';
...
const SagarUjjwalGamingGoalOverlay = () => {
  ...
  useEffect(() => {
    const fetchGoalData = async () => {
      const { data: streamerData } = await supabase
        ...
        .eq('id', SAGARUJJWALGAMING_STREAMER_ID)
        .single();
  ...
  useEffect(() => {
    ...
    const goalChannel = pusher.subscribe('sagarujjwalgaming-goal');
    goalChannel.bind('goal-progress', (data: { currentAmount: number }) => {
      setCurrentAmount(data.currentAmount);
    });

    const settingsChannel = pusher.subscribe('sagarujjwalgaming-settings');
    ...
    return () => {
      if (pusherRef.current) {
        try {
          ...
          pusher.unsubscribe('sagarujjwalgaming-settings');
