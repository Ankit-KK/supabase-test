...
const ArtCreate = () => {
    ...
    const fetchStreamerSettings = async () => {
      try {
        const { data, error } = await supabase
          .rpc('get_streamer_public_settings', { slug: 'artcreate' });
  ...
  const handlePaymentAfterPhone = async () => {
    ...
    try {
      ...
      const response = await supabase.functions.invoke('create-payment-order-artcreate', {
        body: {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background to-muted flex items-center justify-center p-4">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-20 left-20 w-32 h-32 bg-pink-500/10 rounded-full blur-3xl animate-pulse"></div>
        <div className="absolute bottom-20 right-20 w-40 h-40 bg-pink-600/10 rounded-full blur-3xl animate-pulse"></div>
        <div className="absolute top-1/2 left-1/4 w-24 h-24 bg-pink-400/10 rounded-full blur-2xl animate-pulse"></div>
      </div>

      <Card className="w-full max-w-md mx-auto bg-card/95 backdrop-blur-sm border-pink-500/20 shadow-2xl relative overflow-hidden">
        {/* Card glow effect */}
        ...
        <CardHeader className="text-center relative z-10">
          ...
          <CardTitle className="text-2xl font-bold bg-gradient-to-r from-pink-500 to-pink-600 bg-clip-text text-transparent">
            ArtCreate
          </CardTitle>
          <p className="text-muted-foreground text-sm">
            Support ArtCreate with your donation
          </p>
        </CardHeader>
        ...
        <CardContent className="space-y-6 relative z-10">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {donationType === 'message' && (
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  onChange={handleInputChange}
                  className="w-full p-3 border border-pink-500/30 rounded-lg bg-background/50 backdrop-blur-sm focus:border-pink-500 focus:ring-2 focus:ring-pink-500/20 focus:outline-none resize-none"
                  rows={3}
                  maxLength={500}
                  required
                />
            ...
            <Button
              type="submit"
              disabled={isProcessing || sdkLoading}
              className="w-full bg-gradient-to-r from-pink-600 to-pink-500 hover:from-pink-700 hover:to-pink-600 text-white font-semibold py-3 transition-all"
            >
      ...
      <Dialog open={showPhoneDialog} onOpenChange={setShowPhoneDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Please provide your mobile number for payment and order updates
            </DialogDescription>
          </DialogHeader>
