...
const MrChampion = () => {
  ...
  const { pricing } = useStreamerPricing("mr_champion", selectedCurrency);
    ...
    try {
      ...
      if (donationType === "voice" && voiceRecorder.audioBlob) {
        ...
        const { data, error } = await supabase.functions.invoke("upload-voice-message-direct", {
          body: { voiceData: base64, streamerSlug: "mr_champion" },
        });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-[#1a1500] via-[#2d2200] to-[#1a1500]">
      <Card className="w-full max-w-sm backdrop-blur-sm border-[#eab308]/40 shadow-[0_0_30px_rgba(234,179,8,0.2)] relative overflow-hidden bg-gradient-to-br from-[#1a1500] to-[#2d2200]">
        ...
        <CardHeader className="text-center relative z-10 pb-2 flex flex-col items-center">
          ...
          <p className="text-xs text-gray-400">Support Mr Champion with your donation</p>
        </CardHeader>

        <CardContent className="space-y-4 relative z-10">
          <div className="space-y-2">
            <Label className="text-[#facc15]">Your Name *</Label>
            <Input
              name="name"
              value={formData.name}
          ...
          <div className="space-y-2">
            <Label className="text-[#facc15]">Amount *</Label>
            <div className="flex gap-2">
              <Popover open={currencyOpen} onOpenChange={setCurrencyOpen}>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    className="w-[100px] justify-between bg-black/40 text-white border-[#eab308]/30 hover:bg-[#eab308]/10"
                  >
          ...
          {donationType === "text" && (
            <div className="space-y-1">
              <textarea
                ...
                className="w-full min-h-[90px] rounded-md bg-black/40 text-white placeholder:text-gray-400 border border-[#eab308]/30 focus:border-[#facc15] focus:ring-[#facc15]/20 px-3 py-2 text-sm"
          ...
          <DonationPageFooter brandColor="#eab308" />
        </CardContent>
      </Card>
    </div>
  );
