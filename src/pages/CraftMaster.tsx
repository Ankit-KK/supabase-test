...
const CraftMaster = () => {
    ...
    if (orderId && status) {
      if (status === 'PAID') {
        ...
        const uploadVoice = async () => {
          try {
            const { error } = await supabase.functions.invoke('upload-voice-message-craftmaster', {
            ...
            setTimeout(() => navigate('/craftmaster', { replace: true }), 3000);
          }
        };
        uploadVoice();
      } else {
        toast.error('Payment was not completed');
        navigate('/craftmaster', { replace: true });
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('create-payment-order-craftmaster', {
        body: {
      ...
      const checkoutOptions = {
        ...
        returnUrl: `${window.location.origin}/craftmaster?order_id=${data.order_id}&status={order_status}`,
      };
  ...
  if (processingPayment) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#8B4513]/10 to-background flex items-center justify-center p-4">
        <Card className="p-8 max-w-md w-full text-center space-y-4">
          <Loader2 className="w-12 h-12 animate-spin mx-auto text-[#8B4513]" />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#8B4513]/10 to-background py-12 px-4">
      <div className="max-w-2xl mx-auto space-y-8">
        <div className="text-center space-y-4">
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-[#8B4513]/10 mb-4">
            <Music className="w-10 h-10 text-[#8B4513]" />
          </div>
          <h1 className="text-4xl font-bold bg-gradient-to-r from-[#8B4513] to-[#D2691E] bg-clip-text text-transparent">
            Support CraftMaster
          </h1>
          <p className="text-lg text-muted-foreground max-w-xl mx-auto">
            Your donation helps keep the creativity flowing! Add a message or voice note to show your support.
          </p>
        </div>

        <Card className="p-8 space-y-6 border-[#8B4513]/20">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <Input
                ...
                required
                maxLength={100}
                className="text-base border-[#8B4513]/20 focus:border-[#8B4513]"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                rows={4}
                maxLength={charLimit}
                className="text-base resize-none border-[#8B4513]/20 focus:border-[#8B4513]"
              />
            ...
            <div className="flex items-center gap-3 p-4 bg-[#8B4513]/5 rounded-lg border border-[#8B4513]/20">
              <input
                ...
                className="w-5 h-5 rounded border-[#8B4513]/40 text-[#8B4513] focus:ring-[#8B4513] disabled:opacity-50"
              />
            ...
            <Button
              type="submit"
              disabled={loading}
              className="w-full text-lg h-12 bg-[#8B4513] hover:bg-[#A0522D] text-white"
            >
              {loading ? (
                <>
