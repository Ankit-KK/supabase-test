...
import { Pizza, Heart, Sparkles } from "lucide-react";
...
const Streamer17 = () => {
    ...
    const fetchStreamerSettings = async () => {
      try {
        const { data, error } = await supabase
          .rpc('get_streamer_public_settings', { slug: 'streamer17' });
  ...
  const handlePaymentAfterPhone = async () => {
    ...
    try {
      ...
      const response = await supabase.functions.invoke('create-payment-order-streamer17', {
        body: {
      ...
      <Card className="w-full max-w-md mx-auto bg-card/95 backdrop-blur-sm border-orange-500/20 shadow-2xl relative overflow-hidden">
        ...
        <CardHeader className="text-center relative z-10">
          ...
          <p className="text-muted-foreground text-sm">
            Support Streamer 17 with your donation
          </p>
        </CardHeader>
