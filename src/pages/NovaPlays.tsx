...
const NovaPlays = () => {
  ...
  const { pricing } = useStreamerPricing('nova_plays', selectedCurrency);
    ...
    try {
      ...
      if (donationType === "voice" && voiceRecorder.audioBlob) {
        ...
        const { data: uploadResult, error: uploadError } = await supabase.functions.invoke(
          ...
          { body: { voiceData: voiceDataBase64, streamerSlug: "nova_plays" } }
        );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative bg-gradient-to-br from-indigo-950 via-slate-900 to-indigo-900">
      <div className="absolute inset-0 bg-black/20"></div>
      <Card className="w-full max-w-sm mx-auto bg-card/95 backdrop-blur-sm border-indigo-500/20 shadow-2xl relative overflow-hidden">
        <div className="absolute inset-0 bg-black/50"></div>
        <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/20 via-indigo-600/20 to-indigo-400/20 opacity-50 blur-xl"></div>

        <CardHeader className="text-center relative z-10 pb-2">
          <div className="flex justify-center mb-2">
            <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden border-4 border-indigo-500 shadow-xl bg-indigo-600 flex items-center justify-center">
              <span className="text-3xl font-bold text-white">NP</span>
            </div>
          </div>
          <CardTitle className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-indigo-400 to-indigo-600 bg-clip-text text-transparent">
            NovaPlays
          </CardTitle>
          <p className="text-muted-foreground text-xs">Support NovaPlays with your donation</p>
        </CardHeader>

        <CardContent className="space-y-4 relative z-10">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <label htmlFor="name" className="text-sm font-medium text-indigo-400">Your Name *</label>
              <Input id="name" name="name" placeholder="Enter your name" value={formData.name}
                onChange={handleInputChange} className="border-indigo-500/30 focus:border-indigo-500" required />
            </div>

            <div className="space-y-2">
              <label className="text-xs font-medium text-indigo-400">Choose your donation type</label>
                ...
                {(["text", "voice", "hypersound", "media"] as const).map(type => (
                  <button key={type} type="button" onClick={() => handleDonationTypeChange(type)}
                    className={`p-2 rounded-lg border-2 transition-all ${donationType === type
                      ? "border-indigo-500 bg-indigo-500/10" : "border-indigo-500/30 hover:border-indigo-500/50"}`}>
            ...
            <div className="space-y-2">
              <label htmlFor="amount" className="text-sm font-medium text-indigo-400">Amount *</label>
              <div className="flex gap-2">
                <Popover open={currencyOpen} onOpenChange={setCurrencyOpen}>
                  <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" className="w-[100px] justify-between border-indigo-500/30">
                ...
                <Input id="amount" name="amount" type="number" placeholder="Enter amount" value={formData.amount}
                  ...
                  className={`flex-1 border-indigo-500/30 focus:border-indigo-500 ${donationType === "hypersound" ? "opacity-50 cursor-not-allowed" : ""}`}
                  required min="1" />
              </div>
            ...
            {donationType === "text" && (
              <div className="space-y-2">
                <label htmlFor="message" className="text-sm font-medium text-indigo-400">Message (Optional)</label>
                <textarea id="message" name="message" placeholder="Enter your message" value={formData.message}
                  ...
                  className="w-full min-h-[100px] px-3 py-2 rounded-md border border-indigo-500/30 focus:border-indigo-500 bg-background text-foreground" />
                <p className="text-xs text-right text-muted-foreground">{formData.message.length}/{maxMessageLength}</p>
              </div>
            )}
            {donationType === "voice" && (
              <div className="space-y-2">
                <label className="text-sm font-medium text-indigo-400">Voice Message *</label>
                <EnhancedVoiceRecorder controller={voiceRecorder} onRecordingComplete={() => {}}
                  maxDurationSeconds={getVoiceDuration(currentAmount)} brandColor="#6366f1"
                  requiredAmount={pricing.minVoice} currentAmount={currentAmount} />
              </div>
            )}
            {donationType === "hypersound" && (
              <div className="space-y-2">
                <Label className="text-sm font-medium text-indigo-400">Select a Sound</Label>
                <HyperSoundSelector selectedSound={selectedHypersound} onSoundSelect={setSelectedHypersound} />
              </div>
            )}
            {donationType === "media" && (
              <div className="space-y-2">
                <Label className="text-sm font-medium text-indigo-400">Upload Media</Label>
                <MediaUploader streamerSlug="nova_plays"
            ...
            <Button type="submit" className="w-full font-semibold py-6 bg-indigo-600 hover:bg-indigo-700"
