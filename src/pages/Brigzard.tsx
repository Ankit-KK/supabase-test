...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  /* Naval grid — parallax via CSS vars + intensity pulse */
  .nv-grid {
    position: fixed; inset: 0; pointer-events: none; z-index: 1;
    background-image:
  ...
  .nv-crack-burst {
    ...
    animation: nv-crack-burst-anim 0.55s ease forwards;
    filter: blur(0.5px);
  }
  .nv-crack-text {
    position: relative; z-index: 2;
    font-family: 'Black Ops One', cursive; font-size: 44px; color: var(--nv-gold);
    letter-spacing: 0.12em;
    text-shadow: 2px 2px 0 #000, -1px -1px 0 #000, 0 0 20px rgba(196,167,71,0.8), 0 0 40px rgba(196,167,71,0.4);
    animation: nv-crack-text-anim 0.55s ease forwards;
  }
  @keyframes nv-crack-burst-anim {
    0%   { opacity:0; transform:scale(0.2) rotate(-15deg); }
    25%  { opacity:1; transform:scale(1.12) rotate(4deg); }
    65%  { opacity:1; transform:scale(1.04) rotate(-1deg); }
    100% { opacity:0; transform:scale(1.25) rotate(6deg); }
  }
  @keyframes nv-crack-text-anim {
  ...
  .nv-name {
    ...
    animation: nv-glitch 9s infinite, nv-name-glow-cycle 6s ease-in-out infinite;
  }
  /* Hue-cycle neon glow on name: gold → scarlet → steel → gold */
  @keyframes nv-name-glow-cycle {
  ...
  /* ── Greeting tag — appears when name is typed ── */
  .nv-greeting {
  ...
  /* ── Hero sub — speech bubble, pops in on mount ── */
  .nv-hero-sub-bubble {
  ...
  .nv-hero-sub-bubble::before {
    ...
    border-right:8px solid rgba(196,167,71,0.4);
  }
  .nv-hero-sub-bubble::after {
    content:''; position:absolute; left:-6px; top:50%; transform:translateY(-50%);
    border-top:4px solid transparent; border-bottom:4px solid transparent;
    border-right:6px solid rgba(11,26,46,0.92);
  }
  ...
  .nv-rank-badge {
    font-family: 'Black Ops One', cursive; font-size: 9px;
    color: var(--nv-navy); background: var(--nv-gold);
    padding: 2px 7px; letter-spacing: 0.06em;
    clip-path: polygon(4px 0%, 100% 0%, calc(100% - 4px) 100%, 0% 100%);
    box-shadow: 0 0 8px rgba(196,167,71,0.4);
    white-space: nowrap;
  }
  ...
  /* ── Inputs — gold underline sweep + scan flash on focus ── */
  .nv-iw { position: relative; }
  .nv-iw::after {
    content:''; position:absolute; bottom:0; left:0; right:0; height:1px;
    background:var(--nv-gold); transform:scaleX(0); transform-origin:left;
  ...
  .nv-iw input:focus {
    border-color:var(--nv-gold) !important; background:rgba(196,167,71,0.06) !important;
    box-shadow:0 0 0 1px rgba(196,167,71,0.12), 0 0 16px rgba(196,167,71,0.15) !important;
  }
  ...
  /* Hover: brightness lift — neon sign warming */
  .nv-tb:hover:not(.nv-on) .nv-tb-face { filter:brightness(1.18); }

  /* Afterburn on deselect */
  .nv-tb.nv-cooling .nv-tb-face { animation: nv-afterburn 0.65s ease forwards; }
  @keyframes nv-afterburn {
    0%   { box-shadow: 0 0 18px rgba(196,167,71,0.7), 0 0 36px rgba(196,167,71,0.4); }
    100% { box-shadow: none; }
  }
  ...
  /* ── Divider — electricity bolt travelling left→right ── */
  .nv-div { height:1px; background:rgba(74,111,165,0.2); position:relative; overflow:visible; }
  .nv-div::before {
    content: '⚓'; position: absolute; left:50%; top:50%;
    transform:translate(-50%,-50%); font-size:8px; color:rgba(196,167,71,0.5); z-index:2;
  }
  .nv-div::after {
  ...
  .nv-tier {
    ...
    padding:7px 4px; border:1px solid rgba(74,111,165,0.2); background:rgba(16,34,64,0.2);
  ...
  /* ── 3D Deploy button — pulses at rest ── */
  ...
  .nv-btn:hover:not(:disabled) {
    animation:none;
    box-shadow:inset 0 1px 0 rgba(196,167,71,0.22), 0 0 32px rgba(196,167,71,0.55), 0 0 60px rgba(16,34,64,0.4);
    text-shadow:0 0 16px var(--nv-gold);
  }
...
const MissionCompleteOverlay: React.FC<{amount:string;currency:string;onDone:()=>void}> = ({amount,currency,onDone}) => {
  ...
  useEffect(() => {
    const t=setTimeout(()=>{ref.current?.classList.add("nv-kc-exit");setTimeout(onDone,400);},2800);
    ...
    <>
      ...
      <div className="nv-root nv-page">
        ...
        <div ref={wrapRef} className="nv-scale-wrap" style={{transformOrigin:"top center"}}>
          <div ref={cardRef} className={cn("nv-card nv-in", intensityLevel > 0 && `nv-int-${intensityLevel}`)}>
            ...
            <div className="nv-hero">
              ...
              <div className="nv-operator-tag">
                ...
                {/* Greeting tag — appears as name is typed */}
                {showGreeting && (
                ...
                {/* Speech bubble — pops in on mount, hidden once greeting shows */}
                {!showGreeting && (
                    ...
                    {TYPES.map(t=>(
                      ...
                      >
                        <div className="nv-tb-face">
                          ...
                          <span className="nv-tb-name" style={{
