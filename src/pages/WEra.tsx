...
const WEra = () => {
  ...
  const { pricing } = useStreamerPricing("w_era", selectedCurrency);
    ...
    try {
      ...
      if (donationType === "voice" && voiceRecorder.audioBlob) {
        ...
        const { data, error } = await supabase.functions.invoke("upload-voice-message-direct", {
          body: { voiceData: base64, streamerSlug: "w_era" },
        });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-[#0c1929] via-[#1e3a5f] to-[#0c1929]">
      <Card className="w-full max-w-sm backdrop-blur-sm border-[#3b82f6]/40 shadow-[0_0_30px_rgba(59,130,246,0.2)] relative overflow-hidden bg-gradient-to-br from-[#0c1929] to-[#1e3a5f]">
        <div className="absolute inset-0 bg-black/60 backdrop-blur-md" />

        <CardHeader className="text-center relative z-10 pb-2 flex flex-col items-center">
          <div className="w-20 h-20 rounded-full overflow-hidden border-4 border-[#60a5fa] shadow-xl mb-3 bg-[#1e3a5f] flex items-center justify-center">
            <span className="text-3xl font-black text-[#60a5fa]">W</span>
          </div>
          <CardTitle className="text-2xl font-bold bg-gradient-to-r from-[#60a5fa] to-[#3b82f6] bg-clip-text text-transparent tracking-wider">
            W Era
          </CardTitle>
          <p className="text-xs text-gray-400">Support W Era with your donation</p>
        </CardHeader>

        <CardContent className="space-y-4 relative z-10">
          <div className="space-y-2">
            <Label className="text-[#60a5fa]">Your Name *</Label>
            <Input
              name="name"
              value={formData.name}
          ...
          <div className="space-y-2">
            <Label className="text-[#60a5fa]">Amount *</Label>
            <div className="flex gap-2">
              <Popover open={currencyOpen} onOpenChange={setCurrencyOpen}>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    className="w-[100px] justify-between bg-black/40 text-white border-[#3b82f6]/30 hover:bg-[#3b82f6]/10"
                  >
          ...
          {donationType === "text" && (
            <div className="space-y-1">
              <textarea
                ...
                className="w-full min-h-[90px] rounded-md bg-black/40 text-white placeholder:text-gray-400 border border-[#3b82f6]/30 focus:border-[#60a5fa] focus:ring-[#60a5fa]/20 px-3 py-2 text-sm"
