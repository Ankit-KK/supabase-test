...
const Streamer45 = () => {
  ...
  useEffect(() => {
    ...
    const fetchStreamerSettings = async () => {
      const { data } = await supabase.rpc('get_streamer_public_settings', { slug: 'streamer45' });
      if (data?.[0]) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('create-payment-order-streamer45', {
        body: {
          name: formData.name,
      ...
      const checkoutOptions = {
        ...
        returnUrl: `${window.location.origin}/streamer45`,
      };
