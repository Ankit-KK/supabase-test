...
const YogaTime = () => {
    ...
    if (orderId && status) {
      if (status === 'PAID') {
        ...
        const uploadVoice = async () => {
          try {
            const { error } = await supabase.functions.invoke('upload-voice-message-yogatime', {
            ...
            setTimeout(() => navigate('/yogatime', { replace: true }), 3000);
          }
        };
        uploadVoice();
      } else {
        toast.error('Payment was not completed');
        navigate('/yogatime', { replace: true });
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('create-payment-order-yogatime', {
        body: {
      ...
      const checkoutOptions = {
        ...
        returnUrl: `${window.location.origin}/yogatime?order_id=${data.order_id}&status={order_status}`,
      };
  ...
  if (processingPayment) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#27ae60]/10 to-background flex items-center justify-center p-4">
        <Card className="p-8 max-w-md w-full text-center space-y-4">
          <Loader2 className="w-12 h-12 animate-spin mx-auto text-[#27ae60]" />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#27ae60]/10 to-background py-12 px-4">
      <div className="max-w-2xl mx-auto space-y-8">
        <div className="text-center space-y-4">
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-[#27ae60]/10 mb-4">
            <Music className="w-10 h-10 text-[#27ae60]" />
          </div>
          <h1 className="text-4xl font-bold bg-gradient-to-r from-[#27ae60] to-[#229954] bg-clip-text text-transparent">
            Support YogaTime
          </h1>
          <p className="text-lg text-muted-foreground max-w-xl mx-auto">
            Your donation helps bring peace and wellness! Add a message or voice note to show your support.
          </p>
        </div>

        <Card className="p-8 space-y-6 border-[#27ae60]/20">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <Input
                ...
                required
                maxLength={100}
                className="text-base border-[#27ae60]/20 focus:border-[#27ae60]"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                rows={4}
                maxLength={charLimit}
                className="text-base resize-none border-[#27ae60]/20 focus:border-[#27ae60]"
              />
            ...
            <div className="flex items-center gap-3 p-4 bg-[#27ae60]/5 rounded-lg border border-[#27ae60]/20">
              <input
                type="checkbox"
                id="hyperemote"
                ...
                className="w-5 h-5 rounded border-[#27ae60]/40 text-[#27ae60] focus:ring-[#27ae60] disabled:opacity-50"
              />
            ...
            <Button
              type="submit"
              disabled={loading}
              className="w-full text-lg h-12 bg-[#27ae60] hover:bg-[#229954] text-white"
            >
              {loading ? (
                <>
