...
const Eryx = () => {
    ...
    try {
      ...
      if (donationType === "voice" && voiceRecorder.audioBlob) {
        ...
        const { data: uploadResult, error: uploadError } = await supabase.functions.invoke(
          ...
          { body: { voiceData: voiceDataBase64, streamerSlug: "eryx" } }
        );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative bg-gradient-to-br from-orange-950 via-slate-900 to-orange-900">
      <div className="absolute inset-0 bg-black/20"></div>
      <Card className="w-full max-w-sm mx-auto bg-card/95 backdrop-blur-sm border-orange-500/20 shadow-2xl relative overflow-hidden">
        ...
        <CardHeader className="text-center relative z-10 pb-2">
          ...
          <p className="text-muted-foreground text-xs">Support Eryx with your donation</p>
        </CardHeader>
        <CardContent className="space-y-4 relative z-10">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <label htmlFor="name" className="text-sm font-medium text-orange-400">Your Name *</label>
              <Input id="name" name="name" placeholder="Enter your name" value={formData.name}
                onChange={handleInputChange} className="border-orange-500/30 focus:border-orange-500 focus:ring-orange-500/20" required />
            </div>
            <div className="space-y-2">
              <label className="text-xs font-medium text-orange-400">Choose your donation type</label>
              <div className="grid grid-cols-4 gap-1.5">
                {(["text", "voice", "hypersound", "media"] as const).map(type => (
                  <button key={type} type="button" onClick={() => handleDonationTypeChange(type)}
                    className={`p-2 rounded-lg border-2 transition-all ${donationType === type ? "border-orange-500 bg-orange-500/10" : "border-orange-500/30 hover:border-orange-500/50"}`}>
            ...
            <div className="space-y-2">
              <label htmlFor="amount" className="text-sm font-medium text-orange-400">Amount *</label>
              <div className="flex gap-2">
                <Popover open={currencyOpen} onOpenChange={setCurrencyOpen}>
                  <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" className="w-[100px] justify-between border-orange-500/30 hover:bg-orange-500/10">
                ...
                <Input id="amount" name="amount" type="number" placeholder="Enter amount" value={formData.amount}
                  ...
                  className={`flex-1 border-orange-500/30 focus:border-orange-500 ${donationType === "hypersound" ? "opacity-50 cursor-not-allowed" : ""}`}
                  required min="1" />
              </div>
            ...
            {donationType === "text" && (
              <div className="space-y-2">
                <label htmlFor="message" className="text-sm font-medium text-orange-400">Your Message (Optional)</label>
                <textarea id="message" name="message" placeholder="Enter your message" value={formData.message}
                  ...
                  className="w-full min-h-[100px] px-3 py-2 rounded-md border border-orange-500/30 focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 bg-background text-foreground placeholder:text-muted-foreground" />
                <p className="text-xs text-right text-muted-foreground">{formData.message.length}/{maxMessageLength}</p>
              </div>
            )}
            {donationType === "voice" && (
              <div className="space-y-2">
                <label className="text-sm font-medium text-orange-400">Voice Message *</label>
                <EnhancedVoiceRecorder controller={voiceRecorder} onRecordingComplete={() => {}}
                  maxDurationSeconds={getVoiceDuration(currentAmount)} brandColor="#f97316"
                  requiredAmount={pricing.minVoice} currentAmount={currentAmount} />
              </div>
            )}
            {donationType === "hypersound" && (
              <div className="space-y-2">
                <Label className="text-sm font-medium text-orange-400">Select a Sound</Label>
                <HyperSoundSelector selectedSound={selectedHypersound} onSoundSelect={setSelectedHypersound} />
              </div>
            )}
            {donationType === "media" && (
              <div className="space-y-2">
                <Label className="text-sm font-medium text-orange-400">Upload Media</Label>
                <MediaUploader streamerSlug="eryx"
            ...
            <Button type="submit" className="w-full font-semibold py-6 bg-orange-600 hover:bg-orange-700" disabled={isProcessingPayment}>
