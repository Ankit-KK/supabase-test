...
const MackleTVDonationMessages = () => {
  ...
  const [showMessages, setShowMessages] = useState<boolean>(() => {
    ...
    const savedPreference = localStorage.getItem("mackletvShowMessages");
  ...
  useEffect(() => {
    const channel = supabase
      .channel('mackletv-donations-dashboard')
      .on(
        'postgres_changes',
        { 
          event: 'INSERT', 
  ...
  useEffect(() => {
    const savedPref = localStorage.getItem("mackletvShowMessages");
    if (savedPref !== null) {
      setShowMessages(savedPref === "true");
    }
    
    // Store the preference on URL for OBS to check
    const currentLink = sessionStorage.getItem("mackletvObsLink");
    if (currentLink) {
      ...
      sessionStorage.setItem("mackletvObsLink", newLink);
      setObsLink(newLink);
    }
  }, []);
  ...
  const setupObsLink = () => {
    ...
    let storedLink = sessionStorage.getItem("mackletvObsLink");
    
    if (!storedLink) {
      // Generate a new link with a random ID
      const randomId = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
      storedLink = `${window.location.origin}/mackletv/obs/${randomId}?showMessages=${showMessages}`;
      sessionStorage.setItem("mackletvObsLink", storedLink);
    }
  ...
  const regenerateObsLink = () => {
    // Generate a new link with a random ID
    const randomId = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
    const newLink = `${window.location.origin}/mackletv/obs/${randomId}?showMessages=${showMessages}`;
  ...
  const handleToggleMessages = () => {
    ...
    localStorage.setItem("mackletvShowMessages", newValue.toString());
    ...
    <div className="container mx-auto py-8 px-4">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <Button variant="outline" onClick={() => navigate("/mackletv/dashboard")}>
            Back to Dashboard
          </Button>
        </div>
      </div>
...
export default MackleTVDonationMessages;
